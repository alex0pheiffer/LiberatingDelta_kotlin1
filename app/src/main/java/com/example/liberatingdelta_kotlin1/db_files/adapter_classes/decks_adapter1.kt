...
import com.example.liberatingdelta_kotlin1.db_files.User_Decks
import com.example.liberatingdelta_kotlin1.R

class decks_adapter(context: Context?) :
    RecyclerView.Adapter<decks_adapter.dataViewHolder>() {
    ...
    override fun onBindViewHolder(
        ...
        if (lData != null) {
            ...
            holder.dataItemView.text = current.name.toString() + " crds:" + current.length
        } else { // covers the case of data not being ready yet
            holder.dataItemView.text = "No Data"
        }
    }

    fun setlData(vals: List<User_Decks>?) {
        lData = vals
