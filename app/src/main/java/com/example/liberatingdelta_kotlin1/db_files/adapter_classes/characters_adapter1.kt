...
import com.example.liberatingdelta_kotlin1.db_files.User_Characters
import com.example.liberatingdelta_kotlin1.R

class characters_adapter(context: Context?) :
    RecyclerView.Adapter<characters_adapter.dataViewHolder>() {
    ...
    override fun onBindViewHolder(
        ...
        if (lData != null) {
            ...
            holder.dataItemView.text = current.name + " lvl:" + current.level
        } else { // covers the case of data not being ready yet
            holder.dataItemView.text = "No Data"
        }
    }

    fun setlData(vals: List<User_Characters>?) {
        lData = vals
