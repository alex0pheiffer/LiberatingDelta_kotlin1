...
import com.example.liberatingdelta_kotlin1.db_files.User_EQPlayed
import com.example.liberatingdelta_kotlin1.R

class eqplayed_adapter(context: Context?) :
    RecyclerView.Adapter<eqplayed_adapter.dataViewHolder>() {
    ...
    override fun onBindViewHolder(
        ...
        if (lData != null) {
            ...
            holder.dataItemView.text = current.name.toString() + " comp:" + current.completed
        } else { // covers the case of data not being ready yet
            holder.dataItemView.text = "No Data"
        }
    }

    fun setlData(vals: List<User_EQPlayed>?) {
        lData = vals
